...
At each position in the window, scores:
	...
	Bonus for runs of positions with all seqs containing common nucletide - 
		compounds
...
# TODO: Include or not include gaps???



def window_analysis(aln_file, 
				out_file, 
				win_size = 300, 
				pos_bonus_mod = 5, 
				run_bonus_mod = 10, 
				twenty_one_bonus_mod = 100):
	win_score = 0
	max_nuc_count = 0
	...
	while win_pos < seq_len - win_size:
		...
		for pos in range(win_pos, win_pos+win_size):
			...
			try:
				...
				pass
			try:
				max_nuc_count =  nuc_counts[max(nuc_counts.iteritems(), 
		...
		print "Win score = {0}\tPos_bonus_score = {1}\trun_bonus_score={2}\ttwenty_one_bonus_score={3} at position {4}"\
		.format(win_score, tot_pos_bonus, tot_run_bonus, tot_21_bonus, win_pos)	
		win_pos +=1	
	...
	print '\npos with max score is {0}'.format(max(combined_scores.iteritems(), 
												key=operator.itemgetter(1))[0], 
											win_size)
	calc_optimum_seq(aln_seq, max(combined_scores.iteritems(), 
								key=operator.itemgetter(1))[0],
					win_size)
	with open(out_file, 'wb') as csvfile:
		...
		for key,value in combined_scores.iteritems():  
						#prints headers with normalised aligned sRNAs
...
def calc_optimum_seq(aln_seq, win_pos, win_size):
	...
	print "Optimum dsRNA construct seq of length {0} at alignment position {1} is:\n{2}"\
		.format(len(out_seq), win_pos, out_seq)	
...
def main():
	...
	parser.add_argument('-run_bonus','--run_bonus_mod', default = '10', 
					type=int)
...
def run_ana():
	a = comline()
	window_analysis(a.input_alignment, a.output_file, a.window_size, 
				a.pos_bonus_mod, 
				a.run_bonus_mod)
